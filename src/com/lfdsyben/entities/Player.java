...
public class Player extends Entity{
	...
	public int jumpFrames = 50, jumpCur = 0, jumpSpd = 1;
	...
	public void tick() {
		...
		if(shoot) {
			...
			if(hasGun && ammo > 0) {
			ammo--;
			// criar projétil e disparar
			
			//System.out.println("tiro bala");
			int dx = 0;
			int px = 6;
			int dy = 0;
			int py = 6;
	...
	public void checkCollisionBalas() {
		for(int i = 0; i < Game.entities.size(); i++) {
			Entity atual = Game.entities.get(i);
			if(atual instanceof Balas) {
				if(Entity.isColidding(this, atual)) {
					ammo +=10;
	...
	public void checkCollisionKitCura() {
		for(int i = 0; i < Game.entities.size(); i++) {
			Entity atual = Game.entities.get(i);
			if(atual instanceof KitCura) {
				if(Entity.isColidding(this, atual)) {
					life+=10;
					if(life > 100) {
						life = 100;
	...
	public void render(Graphics g) {
		...
		if(dir == right_dir) {
			...
			if(hasGun) {
				...
				g.drawImage(Entity.GUN_RIGHT, this.getX()+5 - Camera.x ,this.getY() -  Camera.y - z, null);
			}
		}else if(dir == left_dir) {
			g.drawImage(leftPlayer[index], this.getX() - Camera.x, this.getY() - Camera.y - z, null);
			if(hasGun) {
				if(dir == left_dir) {
				g.drawImage(Entity.GUN_LEFT, this.getX()-3 - Camera.x ,this.getY() -  Camera.y - z, null);
			}
			}
		}
		...
		if(dir == up_dir) {
			...
			if(hasGun) {
				//desenhar arma p bx
			g.drawImage(Entity.GUN_DOWN, this.getX() - Camera.x ,this.getY()+1 -  Camera.y - z, null);
			}
		}
		}else {
			...
			g.fillOval(this.getX() - Camera.x +8, this.getY() - Camera.y - z +16,8,8);
		}
